...
class Catalogs_and_Brands extends AppConfig {
    ...
    console.log(this.#catalogs_brands_hub);
	...
	render() {
		...
		if (this.#catalogs_brands_hub.length === 1) {
			this.fetch_catalogs(catalogs_api_url).then((result) => {
				self.render_catalongs(result);
			});
		}

		if (this.#brands_hub.length === 1) {
			this.fetch_catalogs(brands_api_url).then((result) => {
