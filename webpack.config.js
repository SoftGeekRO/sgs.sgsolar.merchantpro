...
const path = require('path'),
      ...
      manifestPath = path.resolve(outputPath, 'manifest.json'),
      _publicPath_ = '//cdn.softgeek.ro/sgsolar/merchant_pro/';
...
const appConfig = {
  ...
  output: {
    ...
    path: outputPath,
    publicPath: _publicPath_, // necessary for dynamic loading
    clean: true
  },
      ...
      // Fonts
      {
        ...
        generator: {
          filename: (pathData) => {
            const pkg = getPackageFolderName(pathData.filename);
            const fileName = path.basename(pathData.filename);
            return `webfonts/${pkg}/${fileName}`;
          },
          publicPath: _publicPath_
        }
      }
    ]
  },
...
const loaderConfig = {
  ...
  plugins: [
    new webpack.DefinePlugin({
      ...
      __PUBLIC_PATH__: JSON.stringify(_publicPath_),
